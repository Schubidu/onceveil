...
The Preview deploy explicitly passes `--config wrangler.jsonc --ignore-base-config`.

This is intentional. Worker Previews must use the branch's `previews` block as the source of truth, while ignoring any Preview Base configuration in the Cloudflare dashboard. Passing the config path explicitly also avoids ambiguity with configuration artifacts generated by framework build tooling.
