...
The Preview deploy explicitly passes `--config dist/server/wrangler.json --ignore-base-config`.

This is intentional. The Vite/TanStack build generates `dist/server/wrangler.json`; CI verifies that its Preview `DB` binding points to `onceveil-preview` before deployment. Wrangler then consumes that exact generated deployment config while ignoring any Preview Base configuration in the Cloudflare dashboard.


## Existing Workers Builds projects

Cloudflare Workers connected to Builds before Worker Previews were introduced keep the previous preview model until they are explicitly switched.

Before relying on `previews.d1_databases`:

1. Open the Worker in the Cloudflare dashboard.
2. Go to **Settings → Builds**.
3. In the **Set up Worker Previews** banner, choose **Set up**.
4. Review the Preview variables and bindings.
