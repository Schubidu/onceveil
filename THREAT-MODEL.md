...
Legacy `/s/:id#v1.<base64url-key>` fragments are recognized as the previous format but cannot satisfy the new proof-preparation boundary because they contain no server-verifiable authorization. Onceveil does not fall back to unprotected reveal and never sends the AES key to the server. Migration `0006_expire_legacy_share_links.sql` therefore makes the security upgrade explicit by moving still-`AVAILABLE` rows without a replay key to terminal `EXPIRED` state. No legacy row remains deceptively available but unrevealable.
