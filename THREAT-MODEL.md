...
Creation is insert-only. A duplicate server-issued identifier is retried with a fresh random identifier and must never replace an existing `AVAILABLE` or terminal record.
